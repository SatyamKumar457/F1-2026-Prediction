...
result = pd.read_csv(f"{File_Path}Data/Result.csv")
PRE = pd.read_csv(f"{File_Path}Data/PredictedData.csv")
...
spearman_df = Final[["Driver", "Position", "Predicted_Pos"]].dropna(
    subset=["Position", "Predicted_Pos"]
)

rho, p_value = spearmanr(
    spearman_df["Position"],
    spearman_df["Predicted_Pos"]
)

print(f"Spearman Rank (Final): {rho:.2f}")
print(f"P-value: {p_value:.4f}")
print(f"Drivers evaluated: {len(spearman_df)}")
...
plt.savefig(f'{File_Path}Plots/Result_Correlation_Heatmap.png', dpi=300, bbox_inches='tight')
...
plt.savefig(f'{File_Path}Plots/Result_Feature_RaceResult_Correlation.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'{File_Path}Plots/Position_Difference_by_Driver.png', dpi=300, bbox_inches='tight')
